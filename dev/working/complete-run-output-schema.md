...
6. **Simulation contract consolidation.** Update experiment/simulation_record.py, response_inventory.py, metric_plan.py, scenario_collection.py collection-reference discovery, shared/cross_workflow_leaves.py, workflow leaves, retained readers and tree tests. Embed the five input payloads, publish the final marker after inventory verification and keep the human-facing archive in config/. Re-measure the old task’s baseline assumptions before deciding validation cost. Depends on the agreed §8.3 schema/path/compatibility contract.
