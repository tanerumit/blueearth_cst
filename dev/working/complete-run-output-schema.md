...
- Status: working proposal under first-review revision — not an accepted implementation contract or an implemented schema
- Date: 2026-09-21
...
**First-review hold (2026-09-21).** This tree is suitable as an architectural direction, but §§2–8 are not yet a complete runtime contract. The owner selected preservation of the current `seed: auto` calculation. Its source projection includes elevation bytes and provider-code inventory; the rapid fixture's explicit seed cannot test it. Because the resolved seed contributes to collection identity, preserving this calculation may retain an indirect elevation dependency in WF3 identity. Editing inventoried provider files in P5 can also change the computed seed under the same formula. §10 and P0 must close both consequences before implementation approval. P4 remains additive until P5 can switch the WF3 producer and its consumers together.
...
| D17 | Keep shared orography in project climate data, but put its HydroMT `forcing_elevation_catalog.yml` binding and checked preparation settings in WF4. | WF3 collections remain model-independent; an experiment owns the elevation correction applied when preparing Wflow forcing. | Owner-confirmed direction; cross-workflow identity change needs design review before implementation |
...
The owner selected preservation of the current `seed: auto` calculation. In `generation_plan.py`, `generation-seed-material/1` hashes the current source projection, including ancillary elevation, and the provider-code inventory digest; the integer is the canonical digest modulo `2**31 - 1`. Preserve this formula and its dependency classifications unless P0 explicitly resolves an unavoidable conflict. For identical seed-material bytes it must yield the same integer. Editing an inventoried provider file changes its digest and may change the integer even when the weather-generation algorithm is unchanged. The resolved seed enters `generation_config`, which contributes to collection identity. Thus retaining the formula can preserve an indirect elevation dependence after D17 removes direct WF4 preparation documents. P0 must reconcile that consequence with the proposed model-independent collection boundary and clarify whether numeric seed parity across the P5 code refactor is required; do not claim both invariants without a demonstrated construction. Test an automatic-seed case and compare scientific outputs separately under a matched explicit seed.
...
This changes scientific identity, not only a path. Today WF3's source projection, `collection_id` and collection revision bind the orography and preparation context even though weather generation does not consume them; that projection also affects the automatic seed. Direct WF4 preparation evidence is proposed for removal from the new collection intent, while the new simulation identity includes its checked elevation and preparation settings. The owner has chosen automatic-seed parity. Because the resolved seed is itself identity-bearing, P0 must decide whether an indirect elevation dependency in the new collection identity is acceptable; strict exclusion of every WF4 elevation influence would conflict with the chosen seed calculation unless a further identity/seed mechanism is justified. Resolve this before claiming model independence. The exact new schema labels remain open in §10.

Never edit the inspected sealed collection to demonstrate the proposed tree. Missing or modified shared elevation must cause a clear refusal before dependent WF4 forcing preparation, but not invalidate a new model-independent WF3 collection. Reusing a matching version verifies its checksum instead of replacing it. Standalone experiment export must bundle its checked elevation dependency; the normal preservation unit is the project.
...
|---|---|---|
| Automatic-seed continuity and identity boundary | Preserve `generation-seed-material/1` and its modulo result for identical material; settle whether numerical seed parity must also hold across changed provider-code bytes, and whether indirect elevation influence on collection identity is acceptable | The owner selected the current calculation. The P5 provider edits change inventoried bytes, and strict WF3 identity independence would exclude elevation; the rapid seed is explicit. |
...
| Collection JSON consolidation | Two JSONs in the scenario engine's collection-record directory; four WF3 provenance sections embedded in the intent | Requires identity migration, strict field sets and section canonicalization; WF4 preparation moves to experiment ownership. |
...
| Provider-specific integration | `scenarios/<id>/weathergenr/` holds execution YAML, retained date CSVs and diagnostics; `series/` holds the published climate time series | Define current provider-subtree readers and change path/term consumers from `forcing/`; keep currently temporary realization netCDFs temporary unless explicitly changed. |
| Simulation intent/marker contract | Two records directly under _engine/: immutable embedded-input intent and final completion marker | Set strict fields, section digests and publication order; update current file readers, stored references and declared targets together. |
| Wflow run settings and temporal evidence | `hydrology/wflow/run_settings/` for retained TOMLs; one common temporal object in the response inventory | Preserve per-run verification and update selector paths and hashes. |
...
| Metric-set contract placement | Indicator CSVs and `metric_run_lookup.csv` under `results/metric_sets/<id>/`, both keyed by `run_group_id`; marker and exact benchmark under the single experiment `_engine/metric_sets/<id>/`; environment embedded in the marker | Bind both siblings by full identity and checked paths, retain section digest and benchmark bytes; make a clean metric-column break. |
...
| Collection version / identity migration | New identity projection and fresh-output readers; leave old IDs on disk untouched | Preparation and provider-code changes can legitimately alter new identities; no reader bridge is required. |
...
| Phase emission/reader matrix | P4 adds readers without switching production; P5 first emits the new collection; P6 first emits the new experiment and metric set | A phase that commits new outputs before its Snakefile and WF4 consumers migrate cannot be independently runnable. |
| Clean comparison reference | Capture pre-change auto-seed and matched explicit-seed cases with stated tolerances | Accumulated output trees and the rapid fixture's fixed seed cannot prove scientific continuity. |

These are design choices, not permission to implement them. P0 in the master brief must settle these choices and the fresh-output/baseline transition before the owner accepts a runtime migration target. The source-archive and history changes can be scoped separately from WF3 static scheduling, provided their invocation/reference interfaces agree. P4 may add new collection validators without emitting new outputs; P5 first emits the new collection and P6 first consumes it in WF4.
...
- 2026-09-21 (first-review response) — Recorded the automatic-seed conflict exposed by removing elevation from the WF3 projection and changing provider code; separated matched-seed numerical comparison from the still-open automatic-seed decision. Added a proposed operation-level legacy matrix and a design-only P0 gate before runtime implementation. Clarified that P4 is additive, P5 first emits the new collection and P6 first consumes it in WF4. These edits do not accept the schema or change runtime code.
...
- 2026-09-21 (preparation catalog ownership) — Confirmed that the collection's HydroMT catalog currently binds shared ERA5 orography for WF4 forcing preparation; retained the catalog at the collection root and clarified why it should not be moved beside the shared dataset.
- 2026-09-21 (preparation catalog naming) — Initially proposed `wflow_forcing_ancillary_catalog.yml` at the collection root; superseded by the WF3/WF4 ownership correction below.
- 2026-09-21 (WF3/WF4 elevation boundary) — Moved the proposed HydroMT binding to experiment `run_settings/forcing_elevation_catalog.yml`, left shared orography under project climate data, and removed WF4 preparation from new scenario-collection identity. Required versioned readers and explicit handling of sealed legacy collections; no runtime migration was performed.
...
- 2026-09-21 (metric run-group key) — Replaced `unit_id` with `run_group_id` in both proposed metric CSVs and recorded a clean metric-column break without legacy compatibility readers; existing output bytes remain untouched.
- 2026-09-21 (metric member run key) — Shortened the proposed metric lookup's third column from `member_run_id` to `run_id`; the lookup row still expresses membership in `run_group_id`.
- 2026-09-21 (cross-file consistency audit) — Aligned proposed capacity, width and bundle-plan field names with `run_group_id`; clarified unchanged padded values, distinct member `run_id`, and the D21 exception to legacy metric-CSV reading. Runtime code and existing outputs remain unchanged.
- 2026-09-21 (generator member filenames) — Named temporary weathergenr member netCDFs `run_<id>.nc` in the proposed provider output directory, using the same scenario run IDs as the retained series while preserving separate paths and temporary retention. Legacy `rlz_<n>_st_<n>.nc` names remain current-state evidence, not proposed names.
- 2026-09-21 (direct generator writes) — Recorded the owner's request to implement `run_<id>.nc` at the R root writer itself in the next revision batch, rather than writing legacy filenames and renaming them afterward. No runtime code changed in this documentation step.
