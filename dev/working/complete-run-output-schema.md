...
| D15 | Keep indicator tables under `results/metric_sets/<id>/` and group their machine contracts under the experiment's single `_engine/metric_sets/<id>/`; embed `metric_environment.json` in `metrics.json`. | Users find result tables without provenance sidecars or repeated `_engine/` folders, while each immutable set retains its own ready marker, exact benchmark evidence and unit mapping. | Owner-directed revision; cross-sibling binding and versioned metric readers required |
...
| Metric environment; return_level_benchmark.json; unit_index.csv; indicator tables | Frozen metric environment, exact benchmark evidence, unit mapping and results | Embed the environment in the set's marker; keep benchmark and unit index in the matching experiment-engine set directory, with indicator tables under `results/metric_sets/<id>/`. |
...
| Metric-set contract placement | Indicator CSVs under `results/metric_sets/<id>/`; marker, exact benchmark and unit index under the single experiment `_engine/metric_sets/<id>/`; environment embedded in the marker | Bind both siblings by full identity and checked paths, version marker fields, retain section digest and benchmark bytes, and preserve root-marker legacy readers. |
