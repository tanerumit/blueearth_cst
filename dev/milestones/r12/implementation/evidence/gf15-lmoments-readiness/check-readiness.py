...
def quantile_gate(
    ...
    with localcontext() as context:
        ...
        support = 1 - Interval.point(c) * standardized
        if c != 0 and support.lo <= 0 < support.hi:
            # The quantile/enclosure/support-rounding gates above already passed.
            # A diagnostic-only CDF residual may remain unresolved at an endpoint.
            probability_residual = Interval(Decimal(0), Decimal(1)) - Interval.point(p)
            residual_status = (
                "unresolved_support_interval; conservative CDF range [0,1]"
            )
        else:
            probability_residual = log_cdf_standard(
                standardized, c
            ).exp() - Interval.point(p)
            residual_status = "resolved"
        ...
        return {
            ...
            "probability_residual_diagnostic_status": residual_status,
