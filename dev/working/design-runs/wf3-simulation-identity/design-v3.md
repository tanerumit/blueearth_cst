...
>
> **Review state.** Author self-review is complete. Domain review of the changed
> forcing, response, and metric boundaries and clean-room external review are
> still pending. No implementation or scientific run has been performed for
> this revision.
>
...
> **1,200–1,900 lines**. It retains schemas, invariants, formulas, migration
...
1. `(run_id, variable, location_id)` is unique.
...
```json
{
  ...
  "series": [
    {"run_id": "001", "variable": "q", "location_id": "130000086", "artifact": 0, "native_selector": "<adapter-private selector>", "units": "<units>", "calendar": "<calendar>", "timestep": "P1D", "time_label": "interval_end", "start": "<timestamp>", "end": "<timestamp>", "missing_value": "<encoding>"}
  ],
...
the recorded adapter revision, and checks declared series metadata. A missing or
...
For each Class-B bundle and metric, count finite, valid blocks after applying the
declared temporal and missingness rules. The minimum is:

```text
required = max(
    ceil(blocks_per_return_period × T),
    GEV_IDENTIFIABILITY_FLOOR,
)
```
...
where `T` is the declared return period. R-4 fixes this ratio-based **shape**;
it does not scientifically ratify a number. This design carries v2's proposed,
untested `blocks_per_return_period = 1.0` and
`GEV_IDENTIFIABILITY_FLOOR = 10`. If `actual < required`, reduction raises and
names the metric, unit, return period, required blocks, and actual blocks. It
does not fit, omit, or emit a caveated row.
...
admissible point estimates and carries no Class-B fit interval across this seam.
...
digest, indicator file digests, and evidence labels for validation performed.
...
```text
source inventory + generation config minus seed
    -> resolved seed
...
- Class-B `1.0` and floor `10` are proposed admissibility values, not validated
...
9. Return-level thresholds need methodological validation; passing the proposed
   count gate only establishes admissibility under this design.
...
| GF-18 | pairing claims require evidence | paired fixture with all-empty edges refuses; independent fixture may pass |
...
4. compares each old Class-C pooled value to the mean of new per-run values at
   the same design point and location; and
...
Any unexplained Class-A/B numeric change, Class-C mean mismatch, duplicate join,
or lost row fails the landing. The comparator records config digests, source
...
This table covers all ten `domain-*`, ten `arch-*`, and eleven `risk-*` ids in
the cumulative ledger. It does not change the original reviews or their
dispositions.
