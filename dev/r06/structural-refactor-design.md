...
### Improvement candidates (raised, not applied — per global self-improvement rule)

- **Target:** `design-document` (skill). **Gap:** the decision-record genre has no
  explicit checklist item for "a pure `git mv` commit is not independently runnable
  when references (imports, script paths, `source()` calls, `params:` string paths)
  break on move" — a recurring hazard in any package/module reorg across projects.
  **Proposed change:** add a commit-plan checklist item: "For file-move refactors,
  confirm each move commit is independently runnable, or justify an atomic
  move+reference-rewrite commit / a transitional shim." **Evidence:** this R6
  refactor's central commit-plan tension. **Scope:** applies to any repo doing a
  directory/package reorg. **Suggested placement:** the genre's Commit-plan guidance.
- **Target:** `scientific-workflows` (skill). **Gap:** no explicit warning that a
  Snakemake `--dry-run` gate is **blind** to `params:` string paths, `shell:`/
  `Rscript` command bodies, R `source()` calls, and CLI bootstrap `sys.path` walks —
  so a refactor verified only by `--dry-run` + a dry-run-based `pytest` gate can pass
  green while a runtime-only reference is broken. **Proposed change:** add a
  checklist item: "For move/rename refactors of a Snakemake workflow, add an
  execution-level smoke (one real `shell:`/script invocation) to any gate whose
  correctness depends on a path outside the declared `input:`/`output:` DAG —
  `--dry-run` does not exercise it." **Evidence:** five panel findings (B1, M3,
  arch-4/5/8) shared this root. **Scope:** any Snakemake/polyglot pipeline refactor.
  **Suggested placement:** the smoke-test-gate guidance.
- **Target:** `reproducible-computing` (skill). **Gap:** no explicit rule that a
  run-relative baseline gate must **never write to or restore the tracked baseline
  artifact** — recording the reference to a scratch path avoids a tracked-file
  mutation across a VCS checkout (which can be blocked or contaminate the checkout).
  **Proposed change:** add a checklist item: "A/B baseline gates record the reference
  to a scratch/redirected manifest path; the committed baseline is read-only during a
  gate run — never overwrite-then-restore a tracked artifact across a `git checkout`."
  **Evidence:** external finding ext1-01 on this design. **Scope:** any repo running
  a two-revision reproducibility gate. **Suggested placement:** the manifest/A-B-gate
  guidance.

Implement these improvement candidates? (yes/no)
