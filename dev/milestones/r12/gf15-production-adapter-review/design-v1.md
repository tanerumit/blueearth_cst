# GF15 production adapter — decision record v1

Status: proposed; scientific review and G1 framing choice pending; no implementation authorization  
Date: 2026-09-13  
Decider: owner at G1/G2; author: cst-architect  
Lifecycle: temporary review draft; accepted record becomes frozen-with-supersession in the R12 milestone, with verbatim review archive and append-only revisions. Driver owns promotion and link updates.  
...
Request type: **improve**. Recommend mapping fixed range-normalized L-moment candidate C into WF4 for provisional operational point estimation, together with portable bounded-benchmark evidence. This recommendation is subject to scientific review and G1. Approval of “8x” selected performance policy `gf15-accuracy-8x-v1`; it did not approve this adapter, screening policy, real-bundle adequacy or estimator superiority.

Authoritative inputs are [intake.md](intake.md), [ledger.md](ledger.md), and [status.md](status.md). The ledger has no findings at authoring. Paths below are repository-relative. The frozen method is `dev/milestones/r12/gf15-alternative-estimator-design.md` (accepted v4 D1–D6); integration authority is `dev/milestones/r12/wf3-simulation-identity-design.md` §§7.5–7.6/8.4. The separate adapter record is required by the frozen candidate design; it does not revise that method.
...
| E7 | `gf15-lmoments-readiness/results/attempt-2/` and `results/independent/` contain accepted controls | Production parity is a hypothesis; this draft runs no model, benchmark or network operation |
...
| `orchestrator` | Existing WF4 planning/publication interfaces at assessment commit `d761a65b`; proposed adapter contract v1, no new Snakemake DAG | cst-architect, implementation by python-engineer / independent integration reviewer |
...
## Decision — proposed candidate integration
...
| Moments | Call `lmoments3.lmom_ratios(y,nmom=3)` once for l1,l2,t3. Unbiased order-statistic PWMs: b_r=n⁻¹Σ_i [C(i−1,r)/C(n−1,r)]y_(i), r=0,1,2; l1=b0, l2=2b1−b0, t3=(6b2−6b1+b0)/l2. Refuse nonfinite moments, l2≤0 or abs(t3)≥1. |
...
The finite-mean L-moment domain c>−1 is not a tuned prior. Linear sample L-moments have the stated unbiasedness property under its assumptions; fitted parameters, t3 and quantiles do not inherit it. The pinned implementation defines parity.
...
Add shipped asset `blueearth_cst/experiment/data/gf15-accuracy-8x-v1.json` and loader `blueearth_cst/experiment/return_level_validation.py` (proposed). Load as package resource; no runtime dev import, absolute source-path lookup, network or scratch dependency. Include the asset in distributable contents if packaging requires explicit configuration; test without the source checkout as well as within it.
...
Study Python 3.12.13, NumPy 2.4.6, SciPy 1.18.0 are initial parity targets; record actual build/platform and full numerical closure separately. Add lmoments3 to `current_metric_request` stage-environment roots; retain roots needed by other reducers. Bind installed source hashes in estimator environment as well as versions/artifacts. Platform/native differences need distinct environment records and parity; version equality alone is insufficient. Unavailable exact platform targets return to owner rather than silently relaxing pins.
...
| 2/4 adapter/report | python-engineer: D1–D7 shipped code, owning tests, user docs | model-validator returns parity/fault/diagnostic verdict before integration |
| 3/4 metrics comparison | python-engineer: old/new tables, float64 evidence, upstream inventory | model-validator accepts preserved extraction and documented estimator changes; no model/generation reruns |
| 4/4 integration acceptance | cst-architect reconciles evidence and scientific verdict | named reviewer/owner under accepted gates; baseline/seal separate |
...
| Extraction | Existing annual/rolling/calendar/missingness/partial-year fixtures plus member-boundary-crossing mutant; sample order/hash and usable counts match old extraction. required−1 refuses before library invocation. |
...
| Metrics-only | Disable Wflow/Julia/generation access during retained-response reduction. Collection/revision, simulation, native bytes/request/inventory stay exact. Only new metric-plan/set/report artifacts appear. Non-return-level exported rows remain exact; compare return levels by metric/location/unit keys and document every change/refusal without arbitrary old/new equality thresholds. |
...
Observable gains: new sets identify C and carry verifiable bounded evidence; old sets remain readable; report changes create new identities. Costs: dependency/source guards can prevent fits, reports add size and readers maintain two validation dialects. Return-level differences are expected and documented, never hidden by baseline refresh. No prediction of better real-system accuracy follows.

## Alternatives considered and G1 decision

**A — provisional C integration (recommended):** adopt D1–D8 after review. This explicitly maps the assessed candidate and closes provenance gaps together. Prefer if owner accepts provisional point estimates under permissive, adaptively selected retained-data policy with actual-bundle adequacy unestablished. It imposes dependency/source pinning, compatibility tests and disclosure; no new estimator research.
...
G1 chooses A/B/C and resolves scientific framing findings against this exact v1. Next is mandatory model-validator review, then owner choice. G2 later requires converged named-version review, ledger dispositions and owner acceptance. External-review CLI transport/auth/isolation is driver-owned later; no fallback inferred.
