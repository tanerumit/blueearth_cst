...
| `ext1-3` | blocking | CONFIRMED — the fixture is UNTRACKED, so a contaminated before-state cannot be recovered from git, having never been in git. **Driver erratum 2026-08-25:** this disposition originally read "shared between worktrees". That is wrong and was the driver's addition, not the reviewer's wording — each worktree holds its own copy (distinct inodes). The sharing is across BRANCHES within a worktree, and the finding stands unchanged | **ACCEPTED** | `D-14.3`: promoted from "before G6 depends on it" to a **hard pre-implementation gate** with four ordered steps, including recording the four data targets' hashes into a TRACKED file |
