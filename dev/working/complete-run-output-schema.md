...
| Request `generation/output/rlz_<n>_st_<n>.nc` | Temporary `scenarios/<id>/weathergenr/output/run_<id>.nc` | Name both root and perturbed generator products by the corresponding scenario `run_id`; retain their `temp()` lifecycle. These files are absent from the completed output tree, unlike published `series/run_<id>.nc`. The R root writer, Snakemake declarations and provider inputs must agree on the new paths. |
