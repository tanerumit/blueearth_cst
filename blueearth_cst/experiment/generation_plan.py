...
# The generator's executables; their static repository imports are followed.
...
# Modules the generator imports that cannot change a generated byte (t2609251004).
# They log, archive, snapshot or compose config: composition's RESULT reaches the
# request by value (`settings`, `water_year_start`), and the advanced-settings
# schema's resolved defaults do too. Hashing their source made a WF4 settings
# section or a log-format edit re-key every scenario collection. Excluded files
# are not followed either, so anything reached only through them drops out too;
# tests/test_wf3_helper_boundary.py pins what remains.
GENERATION_INVENTORY_EXCLUDED = (
    "blueearth_cst/shared/advanced_settings.py",
    "blueearth_cst/shared/config_composition.py",
    "blueearth_cst/shared/invocation_history.py",
    "blueearth_cst/shared/provenance.py",
    "blueearth_cst/shared/run_log_core.py",
    "blueearth_cst/shared/windows_job.py",
...
def generation_configuration(config, repository, *, stage=True):
    ...
    code = repository_code_inventory(
        Path(repository), GENERATION_CODE_ENTRIES, GENERATION_INVENTORY_EXCLUDED
    )
    request = {
