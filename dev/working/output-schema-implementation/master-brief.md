...
- Maintain scientific ordering and numerical behavior under a matched explicit seed. P0 must decide whether `seed: auto` preserves its current projection or intentionally changes; do not claim automatic-seed parity from the explicit-seed rapid fixture. Distinguish intentional identity/path differences from scientific changes. Update live references and documentation with each contract change.
...
1. Before runtime implementation, finish P0 and ask the owner to accept the selected contracts in schema §§2–8, including the clean break from pre-migration output schemas and the automatic-seed policy. Agreement with the current working tree alone authorizes design finalization, not runtime implementation.
