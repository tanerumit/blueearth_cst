...
and package recreation instructions. `results/attempt-2/` is the current exclusively
claimed core attempt; completion is conditional on independent scientific review.
The first core attempt at `results/` and its `attempt-1-sources/` snapshot are
retained as superseded evidence after independent review found an endpoint bug.
...
& $StagePython -B "$Readiness/check-readiness.py" run-fixed --output "$Readiness/results/attempt-2" --scratch "$Scratch/final-controls-2" *> "$Scratch/final-controls-2.log"

# Read-only mechanical verification; output must be absent.
& $StagePython -B "$Readiness/independent-review.py" --results "$Readiness/results/attempt-2" --output "$Scratch/mechanical-review-2.json" *> "$Scratch/mechanical-review-2.log"
```
...
Independent review of attempt 1 found that an optional probability residual
incorrectly vetoed the accepted rounding case `(c,loc,scale,p,q)=(60,0,60,.9,1)`.
The endpoint regression was observed red, then green after the diagnostic was
bounded conservatively by the CDF range [0,1] when its support interval straddles
zero. That diagnostic is explicitly marked unresolved; quantile and support-cell
gates must still pass and are unchanged. Attempt 2 freezes this bounded fix.
