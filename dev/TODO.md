...
| t260719a | `setup_constant_pars` CSDMS restoration — scientific-review task (split out of R3). Design **proposed** in [ADR 0001](decisions/0001-restore-wflow-constant-parameters.md): evidence-gated restoration of 13 mappable constants under CSDMS names, drop deprecated `InfiltCapSoil`, keep `KsatHorFrac`; all 15 mappings resolved. Implementation = restored-vs-clean builds, staticmaps/TOML landing asserts, discharge diff, manifest extension + re-record. Moves the baseline. | active | pre-R6 / constant-params | 2026-07-21 | [ADR 0001](decisions/0001-restore-wflow-constant-parameters.md) |
