# WF3 v2.0 — Climate Experiment: ledger-driven sweep (DRAFT v2)

```
Status:     DRAFT v2 — authored for the design-review-loop run `wf3-experiment-v2`;
...
Body budget: The binding rule, declared at creation: **review rounds relocate and
            ...
            append-only `design-v1.md`) did not cover 52 dispositions. The
            overage and the compression order are stated in §17, for G2.
Revisions:
...
- **A4 — Warm-session ≡ cold-process output identity** (P3): 102 files
  ...
  **Scope limit, stated because the fixture cannot close it.** At fixture scale
  ...
  depth at the default. C-7 is consequently specified at `sweep_workers: 1`
  (one session carries all 12 members, 3× P3's depth, at no extra cost), and
  **A4 at production depth — hundreds of members per session — remains open**,
...
| `wflow_worker.jl` (the line protocol, §6.2) | `python-engineer` (Julia surface is ~40 lines, seeded by `run_wflow_batch.jl`) | C-6, GF-1, GF-14 |
| `generate_weather.R` changes (§4.2), `impose_climate_change.R`'s `verbose = TRUE` (§9.1) | `r-developer` | C-16, the perturb identity leg |
...
3. **The generator's scratch `out_dir` splits from the published output dir,
   ...
   *Why the publish step and not a plain relocation.* R07 **ruled** on these two
   ...
   diff. The rename is on our side of the seam (a `file.rename` in
   `generate_weather.R`, no upstream change); the file names change because the
   flat names cannot carry a realization index. §10 carries a row for each, and
   §13 step 6 updates `dev/scripts/semantic_tree_diff.py:357-360`'s exact-file
   path-map rule and its assertion at `tests/test_semantic_tree_diff.py:621-622`
   **in the same commit**.
...
**Runtime overrides are resolved in the Snakefile, not in the runner.** Three
keys must be settable per-invocation for the documented capture procedure (§10a),
the `allow_partial` gate (GF-7) and the corrupt-ledger recovery (§5.3):
`retain_member_artifacts`, `allow_partial`, `sweep_quarantine`. Snakemake's
...
2. **Digests are verified before reading.** For each row, recompute `sha256` of
   the CSV and hard-fail naming the diverged `member_id` on mismatch. The design
   pays for the digest at record time; this is where it is spent. Without it, a
   member CSV corrupted, truncated or hand-edited *after* a successful sweep is
   read silently and published into `Qstats.csv` / `response_long.csv` / the
   baseline manifest — and §7.2 shows the sweep is **not re-entered** in that
   state, so the sweep-entry digest check of §5.3 never runs. This is F8's only
   detection point; GF-15 is its gate.
...
asymmetry cannot be exercised at all — it needs a `run_historical: true`
variant, which this milestone does not create.)*
...
| key | default | meaning |
|---|---|---|
| `workflows.climate_experiment.sweep_workers` | `_cores` (= `-c N`, degrading to 1) | `p`, the number of persistent Julia workers |
...
| `workflows.climate_experiment.worker_max_members` | `0` (= unbounded) | recycle a Julia worker after N members (§6.3) |
...
| `inputs` | object | `succeeded` | `{"baseline": "sha256:…", "st_csv": "sha256:…" \| null, "weagen_template": "sha256:…"}` — the digests of the files this member was **actually computed from**, so reuse can check its *inputs* and not only its outputs (§5.5 condition 5) |
...
- **Identity fields are mandatory.** v1's record carried no `invocation_id`, no
  ...
  **hard-fails** when the completeness record's `invocation_id` differs from the
  one carried by every row of its declared `ledger_final.csv`. The same two
  fields propagate to `indicators/completeness.csv`.
...
touch the ledger; they report outcomes over a `multiprocessing.Queue` and the
parent serializes them (§6.1). Multi-writer interleaving is therefore not a
...
| A row **sequence** for one `member_id` that violates §5.4's transitions — `succeeded` followed by `claimed` with no intervening `quarantined`, two `succeeded` rows, a terminal row with no preceding `claimed` | **Corruption** | Hard error, handled exactly like a corrupt row: named `member_id`, named line numbers, the quarantine command |

That last row is new in v2 and it is cheap insurance. §5.4 declares a state
machine, but a fold that only "takes the last row" evaluates **no** transition
legality, so an impossible history folds to a perfectly valid state — and §8.1's
escalation clause is worded as "any case where … no legal member transition
resolves", which presumes legality is evaluated *somewhere*. Given R-1 ("a ledger
bug silently skips a member") is rated High, transition legality is the cheapest
available detector for that bug class. §13 step 4 carries all four illegal
sequences above as unit-test cases.
...
key, resolved per §4.3), which moves `_state/` to
`_state/quarantine/<invocation_id>/` and starts a fresh ledger — **excluding
`_state/experiment.lock`, which the running workflow holds** and whose release
handler must still find it at the path it claimed (§6.6). Everything else under
`_state/` moves. Quarantine is retention, never deletion (`cst-run-control`
...
| 5 | Every recorded **input** digest (`inputs.baseline`, `inputs.st_csv`, `inputs.weagen_template`) matches the live file, digested once per invocation at sweep start | disk |
...
**stderr is redirected to a file at spawn time — never left as an undrained
pipe.** This is normative, and it closes a hang class the design otherwise has no
exit from. §6.3 deliberately declines a per-member wall-clock ceiling, so if the
slot reads the worker's stdout with a blocking call while stderr is an undrained
`PIPE`, the Julia worker blocks on its first full stderr buffer (64 KB on
Windows) mid-`Wflow.run` and **never answers**: no heartbeat fires, no timeout
fires, and F12's "all workers retire" recovery never fires either, because the
worker has not exited. Wflow is verbose over a ~35 s run, and today's driver
already writes per-member status lines
(`blueearth_cst/experiment/run_wflow_batch.jl:19,22`) which this redesign moves
from stdout to stderr, adding to the volume — so the buffer fills in normal
operation, not in a corner case. The rule: the slot opens the member log file and
passes that **file handle** as the worker's `stderr` at spawn
(`stderr=<handle>`); a dedicated reader thread is the only acceptable
alternative. GF-14 gates it with a stub worker that emits > 1 MB to stderr before
answering.
...
the *floor check only*.** The identity gate (C-7) deliberately runs at
`sweep_workers = 1` and `sweep_workers = 12` to vary session depth, which is the
variable it is testing; those runs make no performance claim and are not
compared against P3-3.
...
failure early — and (b) the rest in realization-major order. *(Rule (a) is inert
on the tracked fixture, which has `ST_START = 1` and therefore no `cst_0`
members; it applies only to a `run_historical: true` configuration.)*
Slots pull. There is
...
|---|---|---|
| Member log | `{exp_dir}/logs/_parts/3.08_run_sweep/rlz_<n>_cst_<m>.log` | the member's four sub-steps: the `Rscript` perturbation stdout/stderr (including §9.1's `verbose = TRUE` inertness warning), the downscale output, the worker's stderr for that member, the verification result |
| Orchestrator log | `{exp_dir}/logs/_parts/3.08_run_sweep/_orchestrator.log` — **inside the member directory**, and it is the rule's `log:` | plan banner, per-member start/finish lines, worker lifecycle events, the finalization summary |
...
release someone else's. `run_sweep.py` verifies the lock is present and records
the same `invocation_id`, but no longer creates or removes it.
...
```
1. verify _state/experiment.lock is held by THIS workflow  (claimed in onstart, §6.6)
...
8. for every member classified RUN: delete its transient set (§6.6) before re-claiming
...
1. **`scripts/sweep_status.py <exp_dir>`** — reads manifest + ledger and prints
   ...
   `--json` for machine use and `--clear-lock` for the stale-lock recovery
   (§6.6). It lives in `scripts/`, not `dev/scripts/`: `AGENTS.md`'s three-homes
...
| F8 | A member CSV on disk diverges from its recorded digest | 1 member | If the sweep runs: quarantine + re-run. **If the sweep is not re-entered** (declared output present and fresh — the common case), the *reduce* catches it: digest verification before reading, hard-fail naming the member (§4.4) |
...
fifth crash-consistency row, which is what makes that clause operative rather
than rhetorical.
...
| **GF-13** partial sweep, fix, retry | run GF-7(a) to a partial finalization; remove the injection guard; re-invoke with **no** config change | `SM --config allow_partial=true`; remove guard; `SM --config allow_partial=true` | the second invocation **re-enters** the sweep rule (`reason: params have changed`, `incomplete_digest`), runs exactly the previously-failed member, writes a complete `ledger_final.csv` and **deletes** `_state/sweep_incomplete.json`; `Qstats.csv` regains its 6th row. Falsified by `Nothing to be done`. A third invocation is a no-op resume (§7.2) |
| **GF-14** worker stderr flood | a stub `wflow_worker.jl` that writes > 1 MB to stderr before answering its first `run` | `SM` with the stub | the sweep completes and no worker blocks; the flood lands in the member log. Falsified by a hang — which is what an undrained `PIPE` produces at the 64 KB Windows buffer (§6.2) |
| **GF-15** member CSV divergence after a clean sweep (F8) | mutate one byte of `hydrology_runs/rlz_1/output/cst_2.csv`, then re-invoke | `SM` | the sweep is **not** re-entered (declared output present and fresh); the **reduce** hard-fails naming `rlz_1/cst_2` and the digest mismatch; `Qstats.csv` is not rewritten. Falsified by a silent republish |
| **GF-16** slot process dies (F4) | while the sweep runs, `Stop-Process -Force` on one *slot* Python pid — not the Julia worker, which is GF-1's easier case | `SM` (background) + PowerShell kill | the orchestrator records that member `failed`/`class=worker`, respawns the slot **and** its worker, re-claims within `member_max_attempts`, and the sweep exits 0; sibling members unaffected |
...
| F | Gate | F | Gate |
|---|---|---|---|
| F1 | GF-6 (fail-loud), GF-7 (`allow_partial`), GF-13 (retry) | F7 | GF-3 |
...
(`tools` drift). GF-1..GF-5 and GF-8 are the intake's named injection classes;
...
whole-tree diff is retained separately at reduced scope:

| | Operand |
|---|---|
| **C-7 (binding)** | After step 4, generation frozen, from the **same** baseline NCs and the same WG-2 grid: run the fixture sweep twice — once at `sweep_workers: 12` (one member per Julia session — the cold-process reference) and once at `sweep_workers: 1` (all 12 members in one session — maximum available depth, 3x what P3 measured). Require tolerance-0 identity on `hydrology_runs/*/output/cst_*.csv`. Nothing else varies, so a diff means session depth changed a result |
...
| **HM-7+** `indicators/completeness.csv` | **new, conditional** | written **only** on the `allow_partial` path; per-cell `state`, `n_present`/`n_expected`, missing member ids, plus `invocation_id` + `config_digest` (§5.2). `indicators/` is pinned by `check_baseline.py:187-189`, so a conditional file there needs an explicit disposition: **it does not join the manifest** (it is absent on every clean run, which would make the slice non-deterministic) | none; its content is gated by GF-7 |
...
| P2 | `B=4` sweep 400.2 s vs `B=1` 619.9 s (−35.4 %), adverse ordering; **3.10-stage makespan 228.1 s at B=4** (`dev/p33/batching-results.md:113-114`), both at `K=12`, `p=3` | The **performance floor**, split into its two comparands: the 228.1 s stage makespan is the *binding* one, the 400.2 s full wall is *reported* (§13 step 8, C-12) |
| P3 | Warm-session ≡ cold-process byte identity, 102 files, tolerance 0, at `B=4` depth | Assumption A4 at **batch** depth. C-7 re-tests it at `sweep_workers: 1` (12 members per session); production depth stays open (§2.2, §15.3) |
...
| H5 | A Julia session stays numerically identical and memory-stable across **production** depth (hundreds of members), not only the 12 the fixture can reach | C-7 at `sweep_workers: 1` bounds it from below only; the real falsifier is the first production sweep (§15.3 names the trigger). `worker_max_members` is the pre-designed mitigation | implementation / first production run |
...
| **C-7** | Session depth does not change results: 12 members in ONE Julia session are byte-identical to 12 members in 12 sessions | any diff at tolerance 0 between the two runs | the paired run of §9.6: `sweep_workers: 12` vs `sweep_workers: 1`, same baseline NCs, `semantic_tree_diff --tolerance 0` over `hydrology_runs/*/output/cst_*.csv`. **Not** a whole-tree diff — see C-7s |
...
| **C-10** | `Qstats.csv` / `basin.csv` / `RT_*.csv` are byte-identical when fed the same member CSVs, **under full completeness**, and a diverged member CSV is refused rather than republished | any byte differs vs the step-0 capture; or a mutated member CSV is consumed silently | `dev/scripts/check_baseline.py check --workflow climate_experiment` + direct diff; GF-15 for the refusal half |
...
| **C-12** | The performance floor holds at fixture scale under a held `p x M = 12`: **(binding)** the 3.08 sweep-stage makespan is <= P3-3's 228.1 s; **(reported, not gated)** the full-workflow wall vs 400.2 s | measured sweep-stage makespan > 228.1 s | §13 step 4's floor run (pool and batch rules both present, same tree); re-confirmed at step 8. The full wall is reported because v2 also restructures generation and prepare, so 400.2 s is no longer a like-for-like comparand |
...
| **C-17** | Under `allow_partial`, an incomplete response-surface cell is **absent**, never under-averaged, zero-filled, or a crash | any `Qstats.csv`/`basin.csv` row exists for a cell whose realization set is incomplete; or `pd.concat([])` raises; or `indicators/completeness.csv` omits the dropped cell | GF-7, both sub-cases |
...
(including the date-CSV publish step of §4.2), the `{rlz_num}` wildcard.
...
crash-consistency rules (§5.3, **all five rows**, the four illegal transition
sequences among them); the state machine (§5.4, both the `member_hash` and the
input-digest invalidation clauses); the five reuse conditions (§5.5); the
`get_override` precedence rule (§4.3); the `os.replace` bounded retry (§5.3); the
lock's pid + creation-time liveness (§6.6); and the line protocol against a
**stub** worker, including GF-14's stderr flood.
*Then:* `run_sweep.py`, `sweep_slot.py`, `wflow_worker.jl`,
`scripts/sweep_status.py`, and the `onstart`/`onsuccess`/`onerror` lock handlers.
...
the `--dry-run` visibility the step removes (§7.5), and GF-2's recovery sequence
names it.
...
Protocol and comparands: §13 step 8, which becomes the confirmation run.
...
the paired identity run (C-7) and the scoped tree diff (C-7s); GF-1..GF-18.
...
- **`p × M` held at `3 × 4 = 12`**, matching P3-3's frozen `-c 3, --threads 4`.
  Run `SM --forceall` with `sweep_workers=3`, `wflow_threads=4`. (This budget
  binds the floor check only; C-7's identity runs deliberately vary depth.)
- **Two comparands, and only one of them is a gate:**
  **(binding)** the 3.08 sweep-stage makespan against P3-3's **228.1 s** 3.10-stage
  makespan at `B=4` (`dev/p33/batching-results.md:113-114`) — the like-for-like
  operand, since it measures the stage the pool replaces;
  **(reported, not gated)** the full-workflow wall against 400.2 s. v2 also
  restructures generation (`RLZ_NUM` parallel jobs instead of one) and prepare, so
  a `--forceall` wall is no longer comparing the same work. v1 gated on the full
  wall while asking for the stage makespan to be reported, which meant a pool that
  won on the stage it changed could still "fail" on a wall it did not, or the
  reverse.
...
check is precisely the decision point that would reinstate it, which is why §13
moves the binding measurement to the **end of step 4**, while the batch rules
still exist and the revert is one step.
...
**Body budget — stated, not absorbed.** v1 = 1 903 lines; **v2 = 2 969 lines, an
overage of ~55 %**. §9.1, §9.4 and §15.2 were *rewritten* rather than extended,
and every superseded passage remains readable in the append-only `design-v1.md`
in the same run directory — which is what the rule's "relocate, never add" clause
asks for — but that offset did not cover 52 dispositions. The growth is
concentrated in normative surfaces, not prose: §5 (schemas and the five reuse
conditions), §8.2 (seven new gates plus the F→GF coverage table), §12 (five new
falsifiers), §13 (a new step and the completed step-6 inventory), and §17's own
change table. **This is a G2 item.** If the accepted body must meet v1's size,
the compressible surfaces in order are: §17's change table (~55 lines, its job
ends at G2), §5.5's 20-row mapping table, §11.2's probe transcripts, and §14's
fourteen alternatives. Compressing anything in §5, §8.2 or §12 would delete
contract.
