# R13 — Config modularization: Design (DRAFT v4)

> **Status: DRAFT v4** — v2 (revised against the internal three-lens panel:
...
> blocking; premises N19–N20, §6.3). The sequence is withdrawn; the migration
> re-run stands with no shortcut (OV-2).
...
driver's framework-feasibility probe (measurements recorded in `status.md`;
...
> **Decision (v4, `probe-1`): the refresh sequence is withdrawn entirely. The
> full re-run is the only offered path.** None of r2's four steps survives as
> a specified mechanism. What survives of them is knowledge, not mechanism:
...
The conservative path — just re-run — is no longer merely the default and
the recommendation: **it is the only documented path, for every project.**
...
| 7 | Docs sweep + `docs/migration-config-tiers.md` (§15.6's re-run cost with N20's bound; **no shortcut is documented** — the refresh sequence was withdrawn by `probe-1`) + `dev/milestones/r13/migration_config-tiers.md`; CI expected-skip baseline note; **baseline re-record last**, after §16.3's falsifier has been read | — (r2's `tests/test_migration_touch_refresh.py` is retired unwritten with the sequence it tested) |
...
- Each T2 file's bytes enter `configuration_inputs_sha256` on **both** the
  ...
  one.
- The design now names every test module it touches (§16.2, 45 existing + 2 new)
...
- **Migrating a project with a completed experiment re-runs the whole stress
  test, with no shortcut** (§15.6, OV-2). Unbudgeted in v1 and a real charge
  against G5's "mechanical, **bounded** migration": rewriting T1's bytes
  re-fires rule 1.06, which moves the wf1 snapshot digest, which re-fires 3.01,
  which rewrites a sentinel that 3.09 and 3.10 declare **bare**, and the
  realization cascade runs. Both attempted mitigations are withdrawn — the bare
  `--touch` by `ext1-4`, r2's refresh sequence by `probe-1`/N19. The bound the
  design can honestly state is N20's: the reaped-chain portion of the re-run is
  pre-existing re-invocation behavior, not a migration cost.
