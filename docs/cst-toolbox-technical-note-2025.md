...
- *n_realizations*: number of stochastic weather realizations to be generated with the weather generator for sampling natural (historical) variability. (**default value: 1**; the key was `Realizations_number` in the 2025 note and `realizations_num` before R14)
...
- *trajectory*: `transient` or `constant`, and **REQUIRED — there is no default**. The transient change applies change factors in a gradually increasing manner, starting from no change in the first analysis year. In contrast, the constant change applies the same change factors over the entire analysis period. The two describe physically different experiments, so the loader refuses an axis that does not say which: guessing would produce a response surface computed under an assumption nobody made. (The key was `Change_step_type` in the 2025 note and the boolean `transient_change` before R14 `C-32`.)
...
- *trajectory*: as for temperature, and equally required.
