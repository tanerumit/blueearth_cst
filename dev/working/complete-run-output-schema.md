...
- Lifecycle: temporary working document; keep the current proposal in place and use Git history for earlier drafts. Promote the accepted contract to dev/reference/ before closing the work.
...
|---|---|---|
| WF3-only automatic-seed projection | Define `generation-seed-material/2` with only WF3 climate, basin, perturbation, generator-settings and generator-code dependencies; retain canonical digest modulo `2**31 - 1` | Strict WF3 independence supersedes v1's elevation and WF4-code inputs. The owner accepts numeric auto-seed changes when material or inventoried generator code changes; P0 must capture a pre-change comparator and prove WF4-only changes do not affect the new seed. |
...
- **2026-09-21:** Current working proposal incorporates the first architecture review and the owner's clean-output and strict-WF3 decisions. Earlier draft changes are recoverable from Git history; this document states only the current target and open P0 contracts. Runtime implementation has not begun.
