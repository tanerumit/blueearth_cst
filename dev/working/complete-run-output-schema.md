...
6. **Simulation contract consolidation and run settings.** Update experiment/simulation_record.py, response_inventory.py, metric_plan.py, downscale_climate_forcing.py, scenario_collection.py collection-reference discovery, shared/cross_workflow_leaves.py, workflow leaves, retained readers and tree tests. Embed the five input payloads, publish the final marker after inventory verification, retain common temporal evidence once and keep the human-facing archive in config/. Re-measure the old task’s baseline assumptions before deciding validation cost. Depends on the agreed §§8.3 and 8.5 schema/path/compatibility contracts.
