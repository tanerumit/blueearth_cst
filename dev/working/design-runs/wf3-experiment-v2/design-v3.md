# WF3 v2.0 — Climate Experiment: ledger-driven sweep (DRAFT v3)

```
Status:     DRAFT v3 — authored for the design-review-loop run `wf3-experiment-v2`;
...
Body budget: The binding rule, declared at creation: **review rounds relocate and
            ...
            mechanism replacement, +7.6 % net. The overage and the compression
            order are stated in §17, for G2.
Revisions:
...
- **G7 — Performance floor:** not worse than P3-3 batching **across the same
  ...
  budget, both legs measured in the same tree (§13 step 8); the at-scale
  advantage stated as a model from measured terms, never claimed as measured.
...
- **Illegal (= corruption), the unit-test set of §13 step 4:**
  ...
  4. any row whose `member_id` is not in the manifest and not a known orphan
     (the row above).
...
(`tools` drift), GF-20 (per-member log separation — G4's gate).
...
| **C-12** | The performance floor holds at fixture scale under a held `p x M = 12`, **across the same scientific boundary** — from a tree with prepare, generation and catalog complete to all 12 member CSVs present: **(binding)** the pool leg's boundary span is <= the batch leg's boundary span, both measured in the same step-4 tree under the same conditions; **(reported, not gated)** component timings per §13 step 8, plus P3-3's 228.1 s / 400.2 s as historical context | measured pool boundary span > measured batch boundary span | §13 step 4's paired floor run (pool and batch rules both present, same tree); re-confirmed at step 8. 228.1 s is no longer the gate: it measures simulate only, while 3.08 fuses perturb + downscale + simulate — gating on it would charge the pool ~156 s of work absent from the comparand (P2) |
...
establish G7 against batching at all. The floor is therefore measured **across
one scientific boundary, both legs in the same step-4 tree**, where the batch
rules still exist:
...
- **The gate: pool span ≤ batch span.** Nothing else is binding.
...
pool's measured same-boundary span lost to batching at fixture scale (C-12) —
...
**v3 = 3 195 (+7.6 % over v2)**. v3's six mechanisms were rewritten *in place*
...
round faulted. The growth remains concentrated in contract, not prose. **This
is a G2 item.** If the accepted body must meet v1's size, the compressible
surfaces in order are: §17's two change tables (~75 lines, their job ends at
G2), §5.5's 20-row mapping table, §11.2's probe transcripts, and §14's fifteen
alternatives. Compressing anything in §5, §6.1, §8.2 or §12 would delete
contract.
