...
def main() -> None:
    ...
    cell = {
        ...
        "accepted_count": 950,
        "valid_rate": 0.95,
        "scale": {"median": -0.3, "median_absolute": 0.4, "p90_absolute": 0.75},
        "relative": {"median": 0.3, "median_absolute": 0.4, "p90_absolute": 0.75},
    ...
    for coordinate in ("scale", "relative"):
        upper[coordinate]["p90_absolute"] = 1.5
    assert score(upper, policy)["individual_gate_pass"]
    upper["relative"]["p90_absolute"] = 1.5000000001
    assert not score(upper, policy)["individual_gate_pass"]
    for field, value in [("median", -0.3000000001), ("p90_absolute", 0.7500000001)]:
        for coordinate in ("scale", "relative"):
