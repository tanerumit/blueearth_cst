...
| D7 | Preserve older sealed output bytes and read their old schemas, except for the explicit D21 metric-column break. | A layout improvement must not rewrite the evidence of a previous assessment; D21 does not require an old metric-CSV reader. | Required migration constraint with D21 exception |
...
| D21 | Use `run_group_id` in both `metric_run_lookup.csv` and `<token>_indicators.csv`, replacing `unit_id`; do not add compatibility readers for the old metric-column contract. | The key identifies one run or a bundle of runs, not a measurement unit; the two tables retain an explicit join. | Owner-directed clean break for this metric-column change; old output bytes are not rewritten |
...
This changes scientific identity, not only a path. Today WF3's source projection, `collection_id` and collection revision bind the orography and preparation context even though weather generation does not consume them. The new collection identity must exclude that WF4-only material; the new simulation identity must include its checked elevation and preparation settings. Specify coordinated canonicalization, publication and versioned reader changes before writing new artifacts. Existing `scenario-collection/1` collections retain their local sidecars, catalog, identities and bytes. A legacy collection used by a new experiment must not silently inherit or discard its checked preparation contract; define an explicit compatibility or refusal path. The exact new schema labels remain open in §10.
...
| preparation_context.json | WF4 simulation intent and elevation catalog (§7) | Legacy collection-owned preparation evidence becomes experiment-owned; preserve old sidecar in sealed collections. |
...
| Collection JSON consolidation | Two JSONs in the scenario engine's collection-record directory; four WF3 provenance sections embedded in the intent | Requires identity migration, strict field sets, section canonicalization and old-sidecar reader dispatch; WF4 preparation moves to experiment ownership. |
...
| Metric-set contract placement | Indicator CSVs and `metric_run_lookup.csv` under `results/metric_sets/<id>/`, both keyed by `run_group_id`; marker and exact benchmark under the single experiment `_engine/metric_sets/<id>/`; environment embedded in the marker | Bind both siblings by full identity and checked paths, retain section digest and benchmark bytes; make a clean metric-column break without an old-`unit_id` reader. |
...
4. **Model-independent collection and shared elevation contract.** Update experiment/scenario_collection.py, collection_resolution.py, scenario_provider.py, generation_plan.py and content_identity.py together with their consumers. Embed four WF3 sidecars, remove WF4 preparation and elevation from new collection identity, and establish old/new reader compatibility before publishing a new collection. Coordinate with WF4's new elevation binding (§7); this is an identity migration, not a packaging-only change.
...
| Reduced collection JSON count | A fresh collection still emits the four WF3 sidecars, embeds WF4 preparation, omits generation evidence, or an old collection stops reading. Assert exactly two JSONs in its scenario engine record directory and exercise versioned readers. |
...
| Stochastic run lineage | A realization lacks a unique empty-`st_id` root, a perturbed row has an unmatched realization or design point, removing `derived_from` changes the intended ancestry, or a legacy table stops reading. Validate the cross-product and compare reconstructed parent IDs against the old rows before accepting a new schema. |
...
| Metric-set grouping | A table or `metric_run_lookup.csv` lacks a checked reference, their `run_group_id` keys do not join, one group has no members or inconsistent grain, the two short-ID directories bind different full IDs, environment-section tampering passes, benchmark bytes are missing or differ, or an orphan is listed as ready. Validate the new contract, metrics-only reuse, paired export and external discovery; no old-`unit_id` reader is required. |
| Cross-file run-group naming | A new config, collection intent, metric plan, lookup, indicator table or reader mixes `unit_id` and `run_group_id`, changes padded values/allocation order, or confuses a group ID with a member `run_id`. Check each proposed producer, digest and consumer together; retain old output bytes without requiring old metric-CSV compatibility. |
| Shared elevation and WF4 binding | A new collection ID changes solely with WF4 orography, a simulation omits its checked elevation/adapter, changed bytes pass, old collections stop reading, or relocation breaks the experiment catalog URI. Test all five. |
...
| Backward compatibility outside D21 | Existing sealed output checksums change or legacy readers for the other versioned contracts cannot load their schema. Snapshot retained files before/after reuse and refusal. The `unit_id`→`run_group_id` metric-column break explicitly has no compatibility reader; old metric tables stay on disk but are not accepted by the new metric CSV contract. |
