...
│   │   ├── wflow_forcing_ancillary_catalog.yml [renamed: WF4 elevation binding]
...
Recommend retaining the catalog at the user-facing collection root under the more specific name `wflow_forcing_ancillary_catalog.yml`, with a relative URI that reaches the shared file from the catalog's location. The legacy name `preparation_catalog.yml` does not say what is being prepared. Today this catalog has one HydroMT `elevation` entry for ERA5 orography, including the adapter settings. WF4 reads the collection's checked catalog, adds the selected scenario series as a forcing entry and writes a temporary per-run catalog for Wflow preparation. The proposed name says that this YAML catalogs ancillary input to Wflow forcing preparation, not the generated scenario series or the final forcing itself. The shared orography file owns its bytes; the collection owns this particular binding and checksum. Putting the YAML beside shared orography would make a shared data directory own a collection-specific preparation contract, and could force distinct collections to share or overwrite one catalog. The engine-side preparation-context reference can no longer assume this catalog is a sibling of collection_intent.json: declare its project-root or collection-data-root anchor explicitly and verify that it resolves to the same checked catalog bytes. The project-relative JSON reference and the catalog URI must resolve to the same checked elevation bytes after project relocation. Verify actual HydroMT path resolution before accepting that choice; do not add CST-specific keys to HydroMT YAML or modify upstream code. Original machine paths can remain provenance, but must not be the only way the retained dependency is resolved.
...
Keep `wflow_forcing_ancillary_catalog.yml` as a separate HydroMT-native input. JSON consolidation does not authorize CST-specific catalog keys or changes in upstream HydroMT code. Version the filename and checked path in the preparation context, and continue reading legacy `preparation_catalog.yml` in sealed collections.
...
- 2026-09-21 (preparation catalog naming) — Renamed the proposed collection YAML to `wflow_forcing_ancillary_catalog.yml` to identify its WF4 role without implying that the catalog contains the scenario series or final forcing; retained the old path for legacy collections.
