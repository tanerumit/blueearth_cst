...
def main() -> None:
    ...
    assert (
        ...
        == "1e7ba148c69352552d298274f9878ecda11c9df2128772fdd7499eb83e5d7e70"
    )
    old, new = read_json(old_path), read_json(here / "criteria.json")
    previous = here.parent / "gf15-accuracy-3x"
    verify_inventory(previous, previous / "artifact-inventory.json")
    verify_inventory(previous / "results", previous / "results/artifact-inventory.json")
    previous_policy = read_json(previous / "criteria.json")
    previous_cells = read_json(previous / "results/individual-cells.json")
    assert (
        read_json(previous / "results/signed-verdict.json")[
            "scientific_gate_conjunction"
        ]
        is False
    )
    ...
    for coordinate in ("scale", "relative"):
        exact(new[f"median_{coordinate}_abs_max"], 0.80)
        exact(new[f"p90_abs_{coordinate}_max"], {"0.9": 4.00, "0.5": 2.00})
    ...
    inputs = [
        ...
        previous / "criteria.json",
        previous / "results/individual-cells.json",
        previous / "results/signed-verdict.json",
        old_path,
    ...
    for method, cells in individual.items():
        ...
        exact([score(cell, previous_policy) for cell in cells], previous_cells[method])
        ...
        comparisons[method] = {
            "original": old_counts,
            "accuracy_3x": counts(previous_cells[method]),
            "accuracy_8x": counts(rescored[method]),
        }
    ...
    write_json(
        ...
        {
            "signed_by": "model-validator /root/model_validator_gf15_8x",
            ...
            "threefold_failed_verdict_preserved": True,
