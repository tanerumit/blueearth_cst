...
│   │   ├── preparation_catalog.yml           [changed URI: collection's WF4 elevation binding]
...
Recommend retaining preparation_catalog.yml at the user-facing collection root, with a relative URI that reaches the shared file from the catalog's location. Today this catalog has one HydroMT `elevation` entry for ERA5 orography, including the adapter settings. WF4 reads the collection's checked catalog, adds the selected scenario series as a forcing entry and writes a temporary per-run catalog for Wflow preparation. The shared orography file owns its bytes; the collection owns this particular binding and checksum. Putting the YAML beside shared orography would make a shared data directory own a collection-specific preparation contract, and could force distinct collections to share or overwrite one catalog. The engine-side preparation-context reference can no longer assume this catalog is a sibling of collection_intent.json: declare its project-root or collection-data-root anchor explicitly and verify that it resolves to the same checked catalog bytes. The project-relative JSON reference and the catalog URI must resolve to the same checked elevation bytes after project relocation. Verify actual HydroMT path resolution before accepting that choice; do not add CST-specific keys to HydroMT YAML or modify upstream code. Original machine paths can remain provenance, but must not be the only way the retained dependency is resolved.
