# Complete-run output tree and record schemas — accepted P0 contract

- Status: P0 contract accepted by the owner on 2026-09-21; runtime implementation is authorized under the phase gates in the master brief, but P1–P7 are not yet implemented
- Date: 2026-09-21
...
- Lifecycle: maintained-current working specification during P1–P7; the versioned review artifacts and finding ledger remain under `design-runs/output-schema-contract/`. Promote the final implemented specification to `dev/reference/` at task closure.

## 1. What this document shows

This is the accepted output-project contract for a successful WF0–WF4 run, compared with the existing layout and generated metadata. “Output tree” means the contents of project_dir, not the Git worktree holding the toolbox. The target contract describes fresh project outputs only; the implementation remains to be done.
...
**Evidence boundary.** No new full WF0–WF4 run was executed for this document. The proposed writers do not exist yet. Current examples were inspected in session-1's test_case/test_local and test_case/test_rapid, alongside their writers. These are accumulated output trees, not evidence of one clean run at the inspected code revision. The local collection c3f88ea5c76f has a ready marker and 14 retained entries currently named forcing; its experiment is named experiment. Rapid supplies examples of WF0 records and wrapper invocation history. Its older retained collections do not have the artifact-local composed snapshot now written by current code. A separate clean pre-change WF3 reference with automatic and explicit seeds completed after design review; §10.9 records its scope and limits.

Decision authority remains in [ADR 0011](../decisions/0011-preserve-config-sources-with-run-records.md). The static-planning boundary remains in [the WF3 intake](t2609191457-wf3-static-planning.md). This document is the maintained working output-schema contract: what a user should find in a completed output project, and what each record should contain. Integrate future layout and schema changes here rather than maintaining competing trees. ADRs, task notes and migration records retain decision history. The owner accepted the selected names, versions and publication rules in §10 on 2026-09-21; implementation must still satisfy the phase gates.

The selected tree incorporates a **seven-to-two reduction in collection JSON files** (§8), a collection-centred `scenarios/` layout with one scenario-level `_engine/` (§3.2), shared elevation (§7), exact source archives (§4), common invocation history (§5), static planning (§6), and consolidation of the simulation family under the experiment engine directory (§8.3). The seven-to-two collection mapping and simulation contract were accepted with the P0 review, not recovered prior approvals. The decision/TODO reconciliation is included in §3.1.

**Reading rule (2026-09-21).** Section 10 is normative and settles the candidate ambiguities in §§2–8. Those sections preserve the layout, payload rationale and prior decision history; their abbreviated examples are not independent competing contracts. The owner first asked to preserve the current `seed: auto` calculation, then required strict WF3 independence when its elevation dependency was identified, and clarified that preserving the formula permits numeric seeds to change. The present seed material includes elevation and a code inventory containing WF4 preparation code, so its inputs must change to honor that boundary. Keep the canonical digest-to-integer method, define and version a WF3-only seed projection, and validate the expected identity and numerical effects. P4 remains additive until P5 can switch the WF3 producer and its consumers together.
...
| Request `generation/output/rlz_<n>_st_<n>.nc` | `scenarios/<id>/series/run_<id>.nc` (roots transit `weathergenr/output/run_<id>.nc`) | Name both root and perturbed generator products by the corresponding scenario `run_id`. Perturbed members are written straight to `series/`; roots are moved there by 3.07. No member netCDF is left under `weathergenr/output/`. In the implementation batch, change the R root writer to write `run_<id>.nc` directly (not write the legacy name and rename it afterward), and align Snakemake declarations, provider inputs and the perturbation writer's output path. |
...
Request type: **improve**. This section states the owner-accepted P0 choices. Sections 2–9 explain the selected layout and retained payloads. Where their illustrative examples omit a field, this section supplies the complete boundary. No runtime implementation or run-control readiness is asserted.
...
ReaderIdentity is forcing-reader-identity/1 from §10.7 with exact nested shapes: driver={name:"raster_xarray",options:{chunks:Maybe<Map<dimension,positive-integer>>,lock:Maybe<boolean>,preprocess:Maybe<"harmonise_dims" or "round_latlon">}}; string driver expands with absent options. data_adapter is null or {rename:Map<text,text>,unit_mult:Map<text,number>,unit_add:Map<text,number>}, absent operations empty. metadata={crs:Maybe<text-or-integer>,nodata:Maybe<Value>,units:Maybe<text>,extent:Maybe<{bbox:Maybe<{West:number,South:number,East:number,North:number}>,time_range:Maybe<{start:text,end:text}>}>}. Normalize absent extent to null and absent bbox/time_range within a present extent to null; bbox numbers are finite with West≤East and South≤North, and time endpoints are ordered ISO 8601 date or date-time strings. unit_interpretation is null for elevation, otherwise {revision:text,variables:[{name,units}]} sorted by name. Native reader evidence retains uri/root, processing/category, cst_unit_interpretation.evidence, and the citation keys license,notes,paper_doi,paper_ref,url,source_license,source_url; exactly those descriptive fields are excluded from scientific identity while their full evidence remains checked. Extent is identity-relevant even when a full-read probe returns the same values, because selected reads may use it. Unknown metadata/control keys refuse; physical descriptor attribute maps above remain complete. Current adapters support rename/unit_mult/unit_add only: newly consumed time conversion/preprocessing requires extension, never silent dropping. Projected ancillary entries={role,sha256,size_bytes,descriptor}, in matching id order; artifact_id resolves uniquely. All six §10.7 preparation document keys are required, with exactly these replacements.
...
| write_netcdf.compression | Compare at least two supported compressed levels (integer 1–9). Exact decoded equality and member/date associations; document permitted container-byte/size differences. Refuse uncompressed choices before collection namespace creation: the installed weathergenr 2.0.1 rejects null and forwards level 0 to ncdf4, which rejects it. No relaxation for rounding: signif_digits remains included. |
...
**Current P0 evidence status: the clean WF3 reference completed and the owner accepted the reviewed contract.** Both isolated predecessor WF3 fixtures finished `all` with exit 0 at source revision `a1a97ffeccea41eed3d1bf449d94f0487121f300`: automatic seed resolved to `1025369264`, and the matched explicit-seed fixture used `123`. Each published a ready `scenario-collection/1` with ten retained forcing files. [The retry report](design-runs/output-schema-contract/prechange-reference-retry.md) records commands, input/catalog/record/series hashes, output roots and numerical fingerprints. The first attempt and its interrupted extraction are retained in [the probe report](design-runs/output-schema-contract/prechange-reference-probes.md). The successful fixtures used a scratch-only catalog pointing to the existing C: ERA5 store because the repository catalog's P: root was unavailable; repository code and catalogs were unchanged. This is a WF3 reference, not a WF0–WF4 continuity result. Accumulated `test_local`/`test_rapid` trees do not substitute for either comparator.

| ID | Status after P0 acceptance | Evidence/result and required observation | Closure |
|---|---|---|---|
| E1 | Observed by source inspection | [Domain review](design-runs/output-schema-contract/internal-review-domain.md) documents predecessor preparation calls, ancillary seed projection and inventoried WF4 code. | P0 inspection; P4 removal still untested |
...
| E3 | Clean WF3 predecessor captured; successor comparison pending | [Retry report](design-runs/output-schema-contract/prechange-reference-retry.md) records completed automatic and explicit-123 runs, hashes and fingerprints. Compare successor decoded series against the explicit-seed reference and capture an independent WF4 comparator below. | P5/P6/P7 comparisons |
...
- **2026-09-21, v2:** Copied v1 before revision and accepted all 14 original internal findings in [ledger.md](design-runs/output-schema-contract/ledger.md). Closed pre-parse launcher coverage, interim phase routes, pinned scientific consumption, creator/candidate reuse, terminal archive recovery, retained rerun evidence, simulation archive and local temporal-reference edges, nested unit-evidence exclusion and path-neutral metric response identity. Required an independent predecessor WF4 comparator. Updated the clean-reference status to the reported stopped extraction attempt; G2 and all scientific-continuity claims remain pending. No runtime code, predecessor design or implementation brief was changed.

- **2026-09-21, v3:** Accepted all three external-round-1 majors in [ledger.md](design-runs/output-schema-contract/ledger.md): exhaustive versioned nested profiles and H/B/ordering/null rules, installed-YAML/provider-argument equivalence to the frozen plan, and excluded-field mutation acceptance with promotion/refusal for scientific effects. Clean break, WF3 independence, exact explicit-seed comparison and the unsatisfied G2 reference gate remain. No runtime code, implementation brief, predecessor design or original review was changed.

- **2026-09-21, accepted P0:** Integrated v3's normative §10 and reviewed tree into this maintained working specification after the owner accepted it. The isolated WF3 automatic and explicit-123 references completed; successor comparisons and the independent predecessor WF4 comparator remain implementation gates. No runtime behavior changed in this documentation revision.

- **2026-09-21, P5 compression amendment:** The owner approved refusing uncompressed `write_netcdf.compression` choices and comparing two supported compressed levels in §10.6. Empirical P5 probing found that installed weathergenr 2.0.1 rejects null and passes level 0 to ncdf4, which rejects it. This amendment changes only the compression exclusion test and admitted execution choices; upstream packages remain unchanged.

- **2026-09-22, P6 reader-metadata amendment:** The owner approved including closed, normalized bbox/time-range extent in `forcing-reader-identity/1` and excluding only seven named citation keys from scientific identity while preserving them in exact full evidence. Installed HydroMT 1.3.1 direct-reader probes found citation edits changed attributes but not decoded values/coordinates; a full-read extent edit also left values unchanged, so the selected-read/full-WF4 checks remain P6 verification gates. No previous new-schema simulation was emitted before this first-version clarification.
