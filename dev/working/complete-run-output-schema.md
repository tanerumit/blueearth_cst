...
**First-review hold (2026-09-21).** This tree is suitable as an architectural direction, but §§2–8 are not yet a complete runtime contract. In particular, changing the WF3 source projection or provider-code inventory can change automatic seeds as well as collection identities; the rapid fixture's explicit seed cannot test that case. §10 lists contract decisions to close before an implementation approval, and the master brief now assigns a design-only P0 for that closure. P4 must remain additive until P5 can switch the WF3 producer and its consumers together.
...
There is a fourth, currently coupled question: which bytes determine `seed: auto`? The current `generation_plan.py` seed projection includes ancillary elevation hashes and provider-code inventory. D17 removes WF4-only elevation from new collection identity, and P5 changes provider/R paths. Without an explicit seed policy these edits can change generated realizations, contradicting a blanket numerical-preservation claim. P0 must select the seed inputs and continuity rule before either code change. Scientific non-regression should compare old and new generation under a matched explicit seed; automatic-seed continuity or deliberate divergence needs its own fixture and acceptance rule.
...
This changes scientific identity, not only a path. Today WF3's source projection, `collection_id` and collection revision bind the orography and preparation context even though weather generation does not consume them; that projection also affects the automatic seed. The new collection identity must exclude WF4-only material; the new simulation identity must include its checked elevation and preparation settings. P0 must specify a separate automatic-seed continuity policy, coordinated canonicalization, publication and new-reader changes before writing new artifacts. The exact new schema labels remain open in §10.
...
|---|---|---|
| Automatic-seed policy and scientific comparator | Choose whether new `seed: auto` retains the current projection or intentionally adopts a new WF3-only projection; compare algorithms under a matched explicit seed | Elevation and provider-code changes can alter realizations, not just names or identities; the rapid seed is explicit. |
